...
const getTransformMatrixString = ( transformationMatrix: TransformationMatrix, getMatrix) => {
...
class PanZoom extends React.Component<Props, State> {
    ...
    componentDidMount(): void {
        const {autoCenter, autoCenterZoomLevel, minZoom, maxZoom,initZoom} = this.props;
        ...
        this.setState({scale: this.props.initZoom})

        let transform = getTransformMatrixString(this.getTransformMatrix(x, y, this.props.initZoom, angle) , null);
        this.setState({transformInit:transform})
    }
    ...
    componentDidUpdate(prevProps: Props, prevState: State): void {
         if (prevProps.initZoom !== this.props.initZoom) {
             this.setState({scale: this.props.initZoom})
         }
         if (prevProps.disabled !== this.props.disabled && this.props.disabled === true ) {
             const {x, y, scale, angle} = this.state;
             const transform = getTransformMatrixString(this.getTransformMatrix(x, y, scale, angle),null);
             this.setState({transform: transform})
         }

        // if (prevState.x !== this.state.x||prevState.y !== this.state.y||prevState.scale !== this.state.scale||prevState.angle !== this.state.angle) {
        //     const {x, y, scale, angle} = this.state;
        //     const transform = getTransformMatrixString(this.getTransformMatrix(x, y, scale, angle), this.props.getMatrix);
        //     this.setState({transform:transform})
        //
        // }

        if (prevProps.center !== this.props.center && this.props.center === true) {
             // this.setState({x: 0,y:0})

             this.autoCenter(this.props.autoCenterZoomLevel)
             this.setState({scale: this.props.initZoom,x: 0,y:0})
         }
    ...
    moveBy = (dx: number, dy: number, noStateUpdate?: boolean = true) => {
        ...
        if (noStateUpdate) {
            ...
            const {boundX, boundY, offsetX, offsetY} = this.getBoundCoordinates({x: transformX, y: transformY}, {
                angle,
                scale,
            ...
            this.transformMatrixString = getTransformMatrixString({a, b, c, d, x: boundX, y: boundY},null);
            ...
            const {boundX, boundY} = this.getBoundCoordinates({x: transformX, y: transformY}, {
                angle,
                scale,
                offsetX: x + dx,
                offsetY: y + dy
    ...
    zoomTo = (x: number, y: number, ratio: number) => {
        ...
        const {boundX, boundY} = this.getBoundCoordinates({x: newX, y: newY}, {
            angle,
            scale,
            offsetX: newX,
            offsetY: newY
    ...
    render() {
        ...
        // const {x, y, scale, angle} = this.state;
        // let transform = getTransformMatrixString(this.getTransformMatrix(x, y, scale, angle),getMatrix);
        ...
        return (
                ...
                >
                    <div
                        ref={elem => this.dragContainer = elem}
                        style={{
